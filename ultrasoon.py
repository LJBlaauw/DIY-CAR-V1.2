...
sm = rp2.StateMachine(
    7, usonic_pio, freq=PIO_FREQ_HZ,
    set_base=Pin(21, Pin.OUT),
    in_base=Pin(20, Pin.IN),
    jmp_pin=Pin(20, Pin.IN),
)
...
def restart_ultrasoon():
    # Eerst netjes stoppen
    try:
        sm.irq(None)
        sm.active(0)
    except:
    ...
    sm = StateMachine(7, usonic_pio_compact,
                            freq=2_000_000,
                            set_base=Pin(TRIG_PIN),
                            in_base=Pin(ECHO_PIN))
    sm.irq(ultra_handler)
    sm.active(1)
