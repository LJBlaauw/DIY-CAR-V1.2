...
# - aanroepen met graden
# - rekent zelf graden -> cm
# - gebruikt ALTIJD state machine 5 (stepper gebruikt SM0..SM4)
# - procedureel, geen klasse
# - wacht op pre-roll
# - FIX: ISR checkt nu eerst of tick_counter > 0 om IndexError te voorkomen
...
ROT_SCALE            = 1.0            # pas dit aan als hij in praktijk net te langzaam/snel is
...
TARGET_SAMPLES_PER_DEG = 3            # ~3 samples per graad
...
def _wait_stepper_done(stepper_mod):
    if not hasattr(stepper_mod, "motorA"):
        return
    while stepper_mod.motorA["active"] or stepper_mod.motorB["active"]:
        pass
...
def _on_pio_irq(sm):
    ...
    # <<< BELANGRIJK: als we klaar zijn, direct weg uit ISR
    if _tick_counter <= 0:
        return
    ...
    if _tick_counter == 0:
        _segment_done = True
        # we laten de PIO nog even lopen; main stopt 'm
...
def scan(dir_char,
    ...
    # 11) terug naar max (altijd omgekeerde richting)
    back_info = None
