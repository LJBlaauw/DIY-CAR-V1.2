"""Hardware test for the data logger from data_logging_updated.md.

All sensor and control values are simulated. The script is intended for
MicroPython on a Pico W/Pico 2 W with ``microdot.py`` in ``/lib``.

Usage:
    1. Start this file on the Pico.
    2. Connect the PC to Wi-Fi ``DIYCAR`` (password ``diycar12345``).
    3. curl http://192.168.4
    4. curl --fail --no-buffer http://192.168.4 -o testrun.csv
    5. curl http://192.168.4

``/status`` shows the current counters. A second simultaneous log client will receive
HTTP 409. Stop curl with Ctrl-C to test reconnection behavior; the run will then remain
active.
"""
...
LOG_PERIOD_MS = 100             # 10 Hz; use 20 for a short 50 Hz test
...
class RecordRing:
    """Non-blocking byte ring that exclusively reads out complete records."""
...
async def simulated_sensor_task():
    """Publishes coherent, recognizably changing values at approximately 50 Hz."""
    sid = 0
    while True:
        sid += 1
        phase = sid * 0.08
        ...
        # Regularly None to test the mandatory empty CSV field.
...
async def index(request):
    return (
        "DIY CAR data-logging test\n"
        "GET /start?run_id=testrun-1\nGET /log.csv\nGET /status\nGET /stop\n",
        200,
...
async def start_endpoint(request):
    ...
    if not valid_id(run_id):
        return "invalid run_id; use letters, numbers, _ or -\n", 400
    if RUN["active"]:
        return "there is already an active run\n", 409
    if LOG_CLIENT_ACTIVE:
        return "wait until the previous log stream is completely closed\n", 409
    start_run(run_id)
    return "run started: %s\n" % run_id
...
async def stop_endpoint(request):
    ...
    stop_run()
    return "run stopped; remaining records can still be read\n"
